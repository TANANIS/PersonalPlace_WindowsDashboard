...
import { dashboardFromLegacy, selectPageCards, selectTopLevelCards } from "../features/dashboard/model";
import { filterDashboardCards } from "../features/search/model";
...
export function AppShell() {
  ...
  useEffect(() => { void loadAppVersion().then(setAppVersion); }, []);
  ...
  const getDashboardState = useCallback(() => stateRef.current, []);
  const {
    query,
    setQuery,
    searchScope,
  ...
  const pageCards = useMemo(
    () => selectPageCards(state, activePage.id),
    [activePage?.id, state.cards],
  );
  const topLevelCards = useMemo(
    () => selectTopLevelCards(pageCards),
    [pageCards],
  );
  ...
  const visibleCards = useMemo(
    () => filterDashboardCards(topLevelCards, pageCards, query),
  ...
  const activeSystemWorkspace = view.kind === "systemWorkspace"
  ...
  return (
    <div className={`app-shell${view.kind === "focusMode" ? " is-focus-mode" : ""}`}>
      ...
      {overlay?.kind === "settings" && <div className="dialog-backdrop" onMouseDown={() => setOverlay(null)}><section ref={settingsDialogRef} tabIndex={-1} className="dialog settings-dialog" role="dialog" aria-modal="true" aria-labelledby="settings-title" onMouseDown={(event) => event.stopPropagation()}><div className="dialog-header"><div><p className="eyebrow">SETTINGS</p><h2 id="settings-title">設定</h2></div><button className="icon-button" onClick={() => setOverlay(null)} aria-label="關閉設定">×</button></div><div className="settings-list"><CalendarSourceManagement /><section className="settings-theme" aria-labelledby="settings-theme-title"><div><strong id="settings-theme-title">{zhTW.appearance.title}</strong><small>{zhTW.appearance.description}</small></div><div className="theme-options">{COLOR_THEMES.map((theme) => <button type="button" key={theme} className={`theme-option theme-${theme}`} aria-label={zhTW.appearance.options[theme]} aria-pressed={colorTheme === theme} onClick={() => setColorTheme(theme)}><span className="theme-preview" aria-hidden="true"><i /><i /><i /></span><strong>{zhTW.appearance.options[theme]}</strong>{colorTheme === theme && <b aria-hidden="true">✓</b>}</button>)}</div></section><button className="settings-row" onClick={() => setOverlay({ kind: "guide" })}><span className="settings-row-icon" aria-hidden="true">?</span><span><strong>{zhTW.guide.settingsTitle}</strong><small>{zhTW.guide.settingsDescription}</small></span><span className="settings-row-arrow" aria-hidden="true">›</span></button><button className="settings-row" onClick={() => setOverlay({ kind: "backup" })}><span className="settings-row-icon" aria-hidden="true">⇅</span><span><strong>備份與還原</strong><small>匯出或取代式還原本機資料</small></span><span className="settings-row-arrow" aria-hidden="true">›</span></button><div className="settings-row cache-row"><span className="settings-row-icon" aria-hidden="true">▧</span><span><strong>縮圖儲存區</strong><small>{cacheInfo ? `${cacheInfo.entries} 個預覽 · ${formatStorageSize(cacheInfo.bytes)}` : "正在讀取使用量…"}</small></span><button className="cache-clear-button" disabled={cacheBusy || !cacheInfo || cacheInfo.entries === 0} onClick={() => void clearStoredPreviews()}>{cacheBusy ? "清除中" : "清除"}</button></div></div><footer className="settings-footer"><span>{zhTW.brand.name}</span><span>版本 {appVersion}</span><span>組建 {buildLabel()}</span></footer></section></div>}

      {overlay?.kind === "guide" && <GuideDialog onClose={() => setOverlay({ kind: "settings" })} />}

      {overlay?.kind === "add" && <AddPanel pageId={overlay.pageId} parentGroupId={overlay.groupId} performIngest={runSerializedIngest} onAdded={() => void refreshDashboard()} onCreateWidget={async (widgetKind, todoListId) => {
        const result = await createWidget(overlay.pageId, overlay.groupId ?? null, widgetKind, todoListId);
        adoptDashboard(result.dashboard);
