...
impl UsageStore {
    pub fn open(path: &Path) -> Result<Self, String> {
        let connection =
            Connection::open(path).map_err(|error| format!("無法開啟使用紀錄資料庫：{error}"))?;
    ...
    pub fn in_memory() -> Result<Self, String> {
        let connection = Connection::open_in_memory()
            .map_err(|error| format!("無法建立暫存使用紀錄資料庫：{error}"))?;
        Self::initialize(connection)
    }
    ...
    pub fn settings(&self) -> Result<TrackingSettings, String> {
        let connection = self
            .connection
            .lock()
            .map_err(|_| "使用紀錄鎖定失敗。".to_string())?;
        connection
            .query_row(
                "SELECT enabled, idle_seconds FROM usage_settings WHERE id=1",
                [],
                |row| {
                    Ok(TrackingSettings {
                        enabled: row.get::<_, i64>(0)? != 0,
                        idle_seconds: row.get(1)?,
                    })
                },
            )
            .map_err(|error| format!("無法讀取追蹤設定：{error}"))
    }

    pub fn update_settings(&self, settings: &TrackingSettings) -> Result<TrackingSettings, String> {
        if ![60, 300, 900, 0].contains(&settings.idle_seconds) {
            return Err("閒置時間只能是 1、5、15 分鐘或不排除。".to_string());
        }
        let connection = self
            .connection
            .lock()
        ...
        connection
            .execute(
                "UPDATE usage_settings SET enabled=?1, idle_seconds=?2 WHERE id=1",
                params![settings.enabled as i64, settings.idle_seconds],
            )
            .map_err(|error| format!("無法更新追蹤設定：{error}"))?;
        Ok(settings.clone())
    }
    ...
    pub fn record(
        ...
        if ended_at <= started_at || app_id == "personal-place" {
            return Ok(());
        }
        ...
        let transaction = connection
            .transaction()
            .map_err(|error| format!("無法儲存使用紀錄：{error}"))?;
        let excluded: bool = transaction
            .query_row(
                "SELECT COALESCE((SELECT excluded FROM usage_apps WHERE app_id=?1), 0)",
                [app_id],
                |row| Ok(row.get::<_, i64>(0)? != 0),
            )
            .map_err(|error| format!("無法讀取 App 設定：{error}"))?;
        ...
        transaction
            .execute(
                "DELETE FROM usage_segments WHERE ended_at < ?1",
                [ended_at - 30 * 24 * 60 * 60],
            )
            .map_err(|error| format!("無法清理舊使用區段：{error}"))?;
        transaction
            .commit()
            .map_err(|error| format!("無法完成使用紀錄：{error}"))
    }
    ...
    pub fn summary(&self, from: i64, to: i64) -> Result<UsageSummary, String> {
        ...
        let apps = statement
            ...
            .map_err(|error| format!("無法讀取使用摘要：{error}"))?
            .collect::<Result<Vec<_>, _>>()
            .map_err(|error| format!("無法整理使用摘要：{error}"))?;
        let total_seconds = apps
            .iter()
        ...
        let segments = statement
            ...
            .map_err(|error| format!("無法讀取使用區段：{error}"))?
            .collect::<Result<Vec<_>, _>>()
            .map_err(|error| format!("無法整理使用區段：{error}"))?;
        Ok(UsageSummary {
            total_seconds,
            apps,
            segments,
    ...
    pub fn clear(&self, app_id: Option<&str>) -> Result<(), String> {
        ...
        if let Some(app_id) = app_id {
            connection
                .execute("DELETE FROM usage_segments WHERE app_id=?1", [app_id])
                .map_err(|error| format!("無法清除使用紀錄：{error}"))?;
            connection
                .execute("DELETE FROM usage_daily WHERE app_id=?1", [app_id])
                .map_err(|error| format!("無法清除每日使用紀錄：{error}"))?;
        } else {
            connection
                .execute("DELETE FROM usage_segments", [])
                .map_err(|error| format!("無法清除使用紀錄：{error}"))?;
            connection
...
pub fn foreground_app() -> Option<(String, String)> {
    ...
    unsafe {
        ...
        let display_name = std::path::Path::new(&path)
            ...
            .to_string_lossy()
            .into_owned();
        if display_name.eq_ignore_ascii_case("personal-workspace")
            || display_name.eq_ignore_ascii_case("personal-place")
        {
            return None;
        }
...
#[cfg(not(windows))]
pub fn install_foreground_hook(_sender: crossbeam_channel::Sender<()>) -> Option<()> {
    None
}
...
mod tests {
    ...
    fn records_only_app_identity_and_prunes_old_segments() {
        let path =
            std::env::temp_dir().join(format!("personal-place-usage-{}.db", std::process::id()));
        let _ = std::fs::remove_file(&path);
        let store = UsageStore::open(&path).unwrap();
        store
            .update_settings(&TrackingSettings {
                enabled: true,
                idle_seconds: 300,
            })
            .unwrap();
        store
            .record("notepad.exe", "記事本", 1_000_000, 1_000_120)
            .unwrap();
