...
describe("TodayWorkspace controlled boundary", () => {
  ...
  it("passes Todo focus requests without launching Place", async () => { const onStartFocus = vi.fn(async () => undefined); const user = userEvent.setup(); render(<TodayWorkspace focusState={null} focusReady={true} focusError={null} onStartFocus={onStartFocus} />); await user.click((await screen.findAllByRole("button", { name: "開始" }))[0]); expect(onStartFocus).toHaveBeenCalledWith({ phase: "focus", linkedTodoId: "todo", linkedGroupId: null }); expect(mocks.launch).not.toHaveBeenCalled(); });
});
