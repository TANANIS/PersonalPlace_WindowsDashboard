...
- 你開啟系統級「活動」工作區時，Tauri 後端只會連線固定的 `http://127.0.0.1:5600`，從本機 ActivityWatch 即時讀取摘要；點擊排行項目或開啟今日時間軸時才會讀取對應的詳細標題／網址。此連線不允許重新導向，也不會改連其他主機。
...
Activity 摘要只在工作區開啟時即時查詢；normalization、internal-page 過濾、低於 60 秒排行過濾與 browser/window 去重只發生在 Personal Place 的 presentation/query layer，不會修改 ActivityWatch 原始資料。
