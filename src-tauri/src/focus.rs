...
impl WorkspaceStore {
    pub fn get_focus_state(&self, now: i64) -> Result<FocusState, String> {
        ...
        let transaction = connection
            .transaction()
            .map_err(|error| format!("無法讀取 Focus 狀態：{error}"))?;
        resolve_expired(&transaction, now)?;
        let result = read_state(&transaction)?;
        transaction
            .commit()
            .map_err(|error| format!("無法完成 Focus 狀態讀取：{error}"))?;
        Ok(result)
    }
    ...
    pub fn start_focus(&self, request: &StartFocusRequest, now: i64) -> Result<FocusState, String> {
        ...
        transaction
            .commit()
            .map_err(|error| format!("無法儲存 Focus：{error}"))?;
        Ok(state)
    }
    ...
    pub fn pause_focus(&self, now: i64) -> Result<FocusState, String> {
        ...
        let transaction = connection
            .transaction()
            .map_err(|error| format!("無法暫停 Focus：{error}"))?;
        resolve_expired(&transaction, now)?;
    ...
    pub fn resume_focus(&self, now: i64) -> Result<FocusState, String> {
        ...
        let transaction = connection
            .transaction()
            .map_err(|error| format!("無法繼續 Focus：{error}"))?;
        let remaining: Option<i64> = transaction
            .query_row(
                "SELECT remaining_seconds FROM focus_state WHERE id = 1",
                [],
                |row| row.get(0),
            )
            .map_err(|error| format!("無法讀取 Focus 剩餘時間：{error}"))?;
    ...
    pub fn stop_focus(&self, outcome: &str, now: i64) -> Result<FocusState, String> {
        if !matches!(outcome, "stopped" | "skipped") {
            return Err("Focus 結束方式無效。".to_string());
        }
        let mut connection = self.lock()?;
        let transaction = connection
            .transaction()
            .map_err(|error| format!("無法結束 Focus：{error}"))?;
...
fn read_state(connection: &rusqlite::Connection) -> Result<FocusState, String> {
    ...
    if state.status == "running" {
        state.remaining_seconds = state
            .ends_at
            .map(|ends| (ends - chrono::Local::now().timestamp()).max(0));
    }
    Ok(state)
}
...
fn phase_seconds(phase: &str, settings: &FocusSettings) -> Result<i64, String> {
    let minutes = match phase {
        ...
        _ => return Err("Focus 階段無效。".to_string()),
    };
    Ok(minutes * 60)
}

fn validate_settings(settings: &FocusSettings) -> Result<(), String> {
    if !(1..=180).contains(&settings.focus_minutes)
        || !(1..=120).contains(&settings.short_break_minutes)
        || !(1..=180).contains(&settings.long_break_minutes)
        || !(1..=12).contains(&settings.long_break_interval)
    {
        return Err("Focus 時間設定超出可用範圍。".to_string());
    }
    Ok(())
}
...
fn resolve_expired(transaction: &rusqlite::Transaction<'_>, now: i64) -> Result<(), String> {
    let status: String = transaction
        ...
        .map_err(|error| format!("無法讀取 Focus：{error}"))?;
    let ends_at: Option<i64> = transaction
        .query_row("SELECT ends_at FROM focus_state WHERE id=1", [], |row| {
            row.get(0)
        })
        .map_err(|error| format!("無法讀取 Focus：{error}"))?;
    if status == "running" && ends_at.is_some_and(|ends| ends <= now) {
        finish_current(transaction, now, "completed")?;
    }
    Ok(())
}
...
fn finish_current(
    ...
    let Some((status, phase, cycle, started, ends, remaining, todo, group)) = row else {
        return Ok(());
    };
    if status == "idle" {
        return Ok(());
    ...
    let planned = ends
        ...
        .unwrap_or(remaining.unwrap_or(0));
    let actual = if status == "running" {
        started
            .map(|start| (now - start).clamp(0, planned))
            .unwrap_or(0)
    } else {
        (planned - remaining.unwrap_or(planned)).max(0)
    };
    ...
    let next_cycle = if phase == "focus" && outcome == "completed" {
        cycle + 1
    } else {
        cycle
    };
...
mod tests {
    ...
    fn pause_resume_and_expiry_use_actual_deadlines_once() {
        ...
        assert_eq!(
            store
                .get_focus_state(start + 600 + 24 * 60 + 99)
                .unwrap()
                .status,
            "idle"
        );
